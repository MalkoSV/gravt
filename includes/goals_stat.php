<?php
    ...
    $db_num = $db->query('SELECT COUNT(*) AS number FROM goals');
    ...
    $db_gp = $db->query('SELECT concat(p.surname," ",p.name) AS name, g.num_g, a.num_a, g.num_g+a.num_a AS gp '
            . 'FROM players p, (SELECT author_id AS scorer, COUNT(*) AS num_g FROM goals GROUP BY author_id) g, '
            . '(SELECT assist_id AS assistant, COUNT(*) AS num_a FROM goals GROUP BY assist_id) a '
    ...
    $db_gpp = $db->query('SELECT concat(p.surname," ",p.name) AS name, '
            ...
            . 'FROM players p, '
            . '(SELECT author_id AS scorer, COUNT(*) AS num_g FROM goals GROUP BY author_id) g, '
            . '(SELECT assist_id AS assistant, COUNT(*) AS num_a FROM goals GROUP BY assist_id) a, '
            . '(SELECT assist2_id AS assistant2, COUNT(*) AS num_a2 FROM goals GROUP BY assist2_id) b '
    ...
    $db_std = $db->query('SELECT COUNT(*) AS number FROM goals WHERE standart=1');
    $std = $db_std->fetch();
    $all_std = $std['number'];
    ...
    $db_half = $db->query('SELECT COUNT(*) AS number FROM goals WHERE half=1');
    $half = $db_half->fetch();
    $half1 = $half['number'];
